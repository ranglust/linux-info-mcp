...
def set_call_ctx(tool: str) -> tuple[str, contextvars.Token]:
    """Set per-call context (tool name, fresh request_id). Returns (rid, token)."""
    rid = uuid.uuid4().hex[:12]
    token = _call_ctx.set({"tool": tool, "request_id": rid})
    return rid, token
