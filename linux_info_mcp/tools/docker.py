...
def handle_docker_ps(args: dict) -> dict:
    ...
    last = args.get("last")
    if last is not None:
        last = validate_int_in_range(last, lo=1, hi=1000, label="last")
...
def handle_docker_logs(args: dict) -> dict:
    ...
    tail = 100 if tail_in is None else validate_int_in_range(tail_in, lo=1, hi=10000, label="tail")
