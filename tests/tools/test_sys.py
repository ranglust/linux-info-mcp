...
def test_lscpu_json_builder():
    assert mod.build_remote_cmd_lscpu(json=True) == "LC_ALL=C lscpu -J"
...
def test_lscpu_rejects_json_and_extended():
    with pytest.raises(ValueError):
        mod.build_remote_cmd_lscpu(json=True, extended=True)


def test_lscpu_rejects_json_and_parseable():
    with pytest.raises(ValueError):
        mod.build_remote_cmd_lscpu(json=True, parseable=True)
...
def test_lsmem_json_builder():
    assert mod.build_remote_cmd_lsmem(json=True) == "LC_ALL=C lsmem -J"
...
def test_lsmem_bytes_builder():
    assert mod.build_remote_cmd_lsmem(bytes=True) == "LC_ALL=C lsmem -b"
...
def test_lsmem_rejects_json_and_summary():
    with pytest.raises(ValueError):
        mod.build_remote_cmd_lsmem(json=True, summary=True)
