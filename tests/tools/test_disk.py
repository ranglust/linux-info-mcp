...
def test_du_all_flags_builder():
    ...
    assert cmd == (
        "LC_ALL=C du -h -s --max-depth=2 --apparent-size -x --threshold=100M -- /var/log"
    )


def test_du_threshold_negative():
    cmd = mod.build_remote_cmd_du(path="/x", threshold="-1G")
    assert cmd == "LC_ALL=C du --threshold=-1G -- /x"
...
def test_lsblk_tree_false_with_json_rejected():
    with pytest.raises(ValueError):
        mod.build_remote_cmd_lsblk(tree=False, json_out=True)


def test_lsblk_tree_false_with_pairs_rejected():
    with pytest.raises(ValueError):
        mod.build_remote_cmd_lsblk(tree=False, pairs=True)
...
def test_lsblk_handler_rejects_leading_dash_device(monkeypatch):
    _stub(monkeypatch, SshResult(b"", b"", 0, False))
    with pytest.raises(ValueError):
...
def test_blkid_handler_rejects_leading_dash_device(monkeypatch):
    _stub(monkeypatch, SshResult(b"", b"", 0, False))
    with pytest.raises(ValueError):
...
def test_smartctl_rejects_leading_dash_device(monkeypatch):
    _stub(monkeypatch, SshResult(b"", b"", 0, False))
    with pytest.raises(ValueError):
...
def test_tools_registry_names():
    ...
    assert names == ["du", "lsblk", "blkid", "smartctl", "blockdev"]
